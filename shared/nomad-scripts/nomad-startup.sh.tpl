...
if [ "$CLOUD_PROVIDER" == "GCP" ]; then
    ...
    /sbin/iptables --wait --insert $docker_chain -i br+ --destination 169.254.169.254/32 -p tcp --dport 53 --jump RETURN # Allow DNS queries
    /sbin/iptables --wait --insert $docker_chain -i br+ --destination 169.254.169.254/32 -p udp --dport 53 --jump RETURN # Allow DNS queries
else
    /sbin/iptables --wait --insert $docker_chain -i docker+ --destination "169.254.0.0/16" --jump DROP
    /sbin/iptables --wait --insert $docker_chain -i br-+ --destination "169.254.0.0/16" --jump DROP
    /sbin/iptables --wait --insert $docker_chain -i docker+ --destination "${vpc_cidr}" --jump DROP
    ...
    /sbin/iptables --wait --insert $docker_chain -i br+ --destination "$dns_server" -p tcp --dport 53 --jump RETURN
    /sbin/iptables --wait --insert $docker_chain -i br+ --destination "$dns_server" -p udp --dport 53 --jump RETURN
